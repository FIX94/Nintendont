...
//Load controller config
	...
	else
	{
		...
		ret = f_open_main_drive( &f, directory, FA_OPEN_EXISTING|FA_READ);
		if(ret != FR_OK)
			ret = f_open_main_drive( &f, "/controller.ini", FA_OPEN_EXISTING|FA_READ);
		else
			dbgprintf("%s was used\r\n", directory);
		if(ret != FR_OK)
			ret = f_open_main_drive(&f, "/controller.ini.ini", FA_OPEN_EXISTING | FA_READ); // too many people don't read the instructions for windows
		if(ret != FR_OK)
