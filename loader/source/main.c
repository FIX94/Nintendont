...
{
    0x48, 0x03, 0x49, 0x04, 0x47, 0x78, 0x46, 0xC0, 0xE6, 0x00, 0x08, 0x70, 0xE1
, 0x2F, 0xFF, 0x1E,
    0x10, 0x10, 0x00, 0x00, 0x00, 0x00, 0x0D, 0x25,
};
/*static const unsigned char AHBAccessPattern[] =
{
        0x68, 0x5B, 0x22, 0xEC, 0x00, 0x52, 0x18, 0x9B, 0x68, 0x1B, 0x46, 0x98,
0x07, 0xDB,
};
static const unsigned char AHBAccessPatch[] =
{
        0x68, 0x5B, 0x22, 0xEC, 0x00, 0x52, 0x18, 0x9B, 0x23, 0x01, 0x46, 0x98,
0x07, 0xDB,
};*/
...
static const char ARGSBOOT_STR[9] ALIGNED(0x10) = {'a','r','g','s','b','o','o','
t','\0'}; //makes it easier to go through the file
static const char NIN_BUILD_STRING[] ALIGNED(32) = NIN_VERSION_STRING; // Versio
n detection string used by nintendont launchers "$$Version:x.xxx"
 ...
 * @param CurDICMD      [in] DI command. (0 == disc image, DIP_CMD_NORMAL == Gam
eCube disc, DIP_CMD_DVDR == DVD-R)
 * @param ISOShift      [out,opt] ISO Shift. (34-bit rshifted byte offset)
 * @param BI2regionOut  [out,opt] bi2.bin region code.
 * @return 0 on success; non-zero on error.
 */
static u32 CheckForMultiGameAndRegion(unsigned int CurDICMD, u32 *ISOShift, u32
*BI2regionOut)
{
        char GamePath[260];
        ...
        else if (IsSupportedFileExt(ncfg->GamePath) || wiiVCInternal)
        {
                ...
                {
                        snprintf(GamePath, sizeof(GamePath), "%s:%s", GetRootDev
ice(), ncfg->GamePath);
                        fres = f_open_char(&f, GamePath, FA_READ|FA_OPEN_EXISTIN
G);
                        if (fres != FR_OK)
                        {
                                // Error opening the file.
                ...
                static const uint8_t CISO_MAGIC[8] = {'C','I','S','O',0x00,0x00,
0x20,0x00};
                if (!memcmp(MultiHdr, CISO_MAGIC, sizeof(CISO_MAGIC)) && !IsGCGa
me(MultiHdr))
                {
                        ...
                        if (BI2regionOut)
                        {
                                if(wiiVCInternal)
                                {
                                        ...
                                        read = WDVD_FST_Read(wdvdTmpBuf, sizeof(
*BI2regionOut));
                                        memcpy(BI2regionOut, wdvdTmpBuf, sizeof(
*BI2regionOut));
                                }
                                else
                                {
                                        f_lseek(&f, 0x8458);
                                        f_read(&f, BI2regionOut, sizeof(*BI2regi
onOut), &read);
                                }
        ...
        else
        {
                ...
                snprintf(GamePath, sizeof(GamePath), "%s:%ssys/bi2.bin", GetRoot
Device(), ncfg->GamePath);
        ...
        for (i = 0x10; i < 0x40 && gamecount < 15; i++)
        {
                ...
                if (TmpOffset > 0)
                {
                        ...
                        else
                        {
                                ...
                                // If the value isn't a multiple of 4, it's unus
able.
                        ...
                        if(CurDICMD)
                                ReadRealDisc(GameHdr, RealOffset, 0x800, CurDICM
D);
                        else
                        {
                                if(wiiVCInternal)
        ...
        while (true)
        {
                ...
                if( redraw )
                {
                        ...
                        PrintFormat(DEFAULT_SIZE, BLACK, subheader_x, MENU_POS_Y
 + 20*3,
                                    "Select a game from this multi-game disc:");
                        for (i = 0; i < gamecount; ++i)
                        {
                                const u32 color = gameIsUnaligned[i] ? MAROON :
BLACK;
                                PrintFormat(DEFAULT_SIZE, color, MENU_POS_X, MEN
U_POS_Y + 20*4 + i * 20, "%50.50s [%.6s]%s",
                                            gi[i].Name, gi[i].ID, i == PosX ? AR
ROW_LEFT : " " );
                        }
                        GRRLIB_Render();
                        Screenshot();
...
//      u64 timeout = 0;
        ...
        bool argsboot = false;
        if(argc > 1) //every 0x00 gets counted as one arg so just make sure its
more than the path and copy
        {
                memcpy(ncfg, argv[1], sizeof(NIN_CFG));
                UpdateNinCFG(); //support for old versions with this
                if(ncfg->Magicbytes == 0x01070CF6 && ncfg->Version == NIN_CFG_VE
RSION && ncfg->MaxPads <= NIN_CFG_MAXPAD)
                {
                        if(ncfg->Config & NIN_CFG_AUTO_BOOT)
                        {       //do NOT remove, this can be used to see if nint
endont knows args
        ...
        *(vu32*)0x932C0494 = CONF_GetSensorBarPosition();
        ...
        if(!isWiiVC)
        {
                ...
                {
                        if( memcmp( (void*)(u), FSAccessPattern, sizeof(FSAccess
Pattern) ) == 0 )
                        {
                                //gprintf("FSAccessPatch:%08X\r\n", u );
                                memcpy( (void*)u, FSAccessPatch, sizeof(FSAccess
Patch) );
                ...
                if (LoadKernel() < 0)
                {
                        // NOTE: Attempting to initialize controllers here cause
s a crash.
                        // Hence, we can't wait for the user to press the HOME b
utton, so
                        // we'll just wait for a timeout instead.
                        PrintFormat(DEFAULT_SIZE, MAROON, MENU_POS_X, MENU_POS_Y
 + 20*20, "Returning to loader in 10 seconds.");
                        UpdateScreen();
                        VIDEO_WaitVSync();
        ...
        *(vu32*)0x92FFFFC0 = isWiiVC; //cant be detected in IOS
        ...
        while(1)
        {
                ...
                if((STATUS_LOADING > 0 || abs(STATUS_LOADING) > 1) && STATUS_LOA
DING < 20)
                {
        ...
        if (!devices[DEV_SD] && !devices[DEV_USB])
        {
                ...
                PrintFormat(DEFAULT_SIZE, MAROON, MENU_POS_X, 232, "No FAT devic
e found!");
                ExitToLoader(1);
        }
        ...
        if(argc > 0 && argv != NULL && argv[0] != NULL)
        {
                ...
                if (first_slash != NULL) strncpy(launch_dir, argv[0], first_slas
h-argv[0]+1);
        }
...
*)0x93100000, (char*)0x93100000 + 0x4D000);
 ...
 for wiimote re-synch
                {
                        if (i == 0) {
                                PrintInfo();
                                PrintFormat(DEFAULT_SIZE, BLACK, 320 - 90, MENU_
POS_Y + 20*10, "B: Cancel Autoboot");
                                GRRLIB_Render();
                                ClearScreen();
                        }
        ...
        bool progressive_scan_enabled = (CONF_GetProgressiveScan() > 0) && VIDEO
_HaveComponentCable();
        if(progressive_scan_enabled) //important to prevent blackscreens
                ncfg->VideoMode |= NIN_VID_PROG; // Set the progressive flag in
VideoMode
        else
                ncfg->VideoMode &= ~NIN_VID_PROG; // Clear the progressive flag
in VideoMode
...
//Init DI and set correct ID if needed
        ...
        if( memcmp(ncfg->GamePath, "di", 3) == 0 )
        {
                ...
                {
                        if(WDVD_Init() != 0)
                                ShowMessageScreenAndExit("The Wii VC Disc could
not be initialized!", 1);
                        if(WDVD_OpenDataPartition() != 0)
                                ShowMessageScreenAndExit("Found no Partition on
Wii VC Disc!", 1);
                        if(!WDVD_FST_Mount())
                                ShowMessageScreenAndExit("Unable to open Partiti
on on Wii VC Disc!", 1);
                        if(WDVD_FST_OpenDisc(0) != 0)
                                ShowMessageScreenAndExit("No game.iso on Wii VC
Disc!", 1);
                        u8 *DIBuf = memalign(32,0x800);
                        if(WDVD_FST_Read(DIBuf, 0x800) != 0x800)
                        {
                                free(DIBuf);
                                ShowMessageScreenAndExit("Cant read game.iso sta
rt!", 1);
                        }
                        if( IsGCGame(DIBuf) == false )
                        {
                                ...
                                {
                                        free(DIBuf);
                                        ShowMessageScreenAndExit("game.iso is no
t a GC Disc!", 1);
                                }
                        }
                ...
                else
                {
                        ...
                        {
                                ShowMessageScreenAndExit("The Disc Drive could n
ot be initialized!", 1);
                        }
                        DI_Close();
                        ...
                        if( IsGCGame(DIBuf) == false )
                        {
                                ...
                                {
                                        free(DIBuf);
                                        ShowMessageScreenAndExit("The Disc in th
e Drive is not a GC Disc!", 1);
                                }
                        }
                        memcpy(&(ncfg->GameID), DIBuf, 4);
                        free(DIBuf);
        ...
        if(SaveSettings)
        {
                ...
                // Write config to the boot device, which is loaded on next laun
ch.
                FIL cfg;
                if (f_open_char(&cfg, "/nincfg.bin", FA_WRITE|FA_OPEN_ALWAYS) ==
 FR_OK)
                {
                        // Reserve space in the file.
                ...
                // Write config to the game device, used by the Nintendont kerne
l.
                char ConfigPath[20];
                snprintf(ConfigPath, sizeof(ConfigPath), "%s:/nincfg.bin", GetRo
otDevice());
                if (f_open_char(&cfg, ConfigPath, FA_WRITE|FA_OPEN_ALWAYS) == FR
_OK)
                {
                        // Reserve space in the file.
                        if (f_size(&cfg) < sizeof(NIN_CFG)) {
 ...
 "CheckForMultiGameAndRegion() failed: %d", ret);
                switch (ret) {
                        case 1:
                                PrintFormat(DEFAULT_SIZE, MAROON, MENU_POS_X, ME
NU_POS_Y + 20*5, "Unable to open the %s.",
                                        CurDICMD == 0 ? "disc image file" : "dis
c drive");
                                break;
                        case 2:
                                PrintFormat(DEFAULT_SIZE, MAROON, MENU_POS_X, ME
NU_POS_Y + 20*5, "Unable to read the disc header.");
                                break;
                        case 3:
                                PrintFormat(DEFAULT_SIZE, MAROON, MENU_POS_X, ME
NU_POS_Y + 20*5, "Unable to read the CISO bi2.bin area.");
                                break;
                        case 4:
                                PrintFormat(DEFAULT_SIZE, MAROON, MENU_POS_X, ME
NU_POS_Y + 20*5, "Unable to open the extracted FST bi2.bin file.");
                                break;
                        case 5:
                                PrintFormat(DEFAULT_SIZE, MAROON, MENU_POS_X, ME
NU_POS_Y + 20*5, "Unable to read the extracted FST bi2.bin file.");
                                break;
                        default:
                                PrintFormat(DEFAULT_SIZE, MAROON, MENU_POS_X, ME
NU_POS_Y + 20*5, "Unknown error code.");
                                break;
                }
                ...
                if (CurDICMD) {
                        ...
                        switch (CurDICMD) {
                                ...
                                case DIP_CMD_DVDR:
                                        ...
                                        break;
                                default:
                                        snprintf(unkdev, sizeof(unkdev), "Unknow
n (CMD: 0x%02x)", CurDICMD);
                                        device = unkdev;
                                        break;
                        }
                        PrintFormat(DEFAULT_SIZE, MAROON, MENU_POS_X, MENU_POS_Y
 + 20*7, "Device: %s", device);
                } else {
                        PrintFormat(DEFAULT_SIZE, MAROON, MENU_POS_X, MENU_POS_Y
 + 20*7, "Filename: %s:%s", GetRootDevice(), ncfg->GamePath);
                }
...
//Set Language
        ...
        if(ncfg->Config & NIN_CFG_MEMCARDEMU)
        {
                ...
                char MemCard[32];
                snprintf(MemCard, sizeof(MemCard), "%s/%s.raw", BasePath, MemCar
dName);
                ...
                if (f_open_char(&f, MemCard, FA_READ|FA_OPEN_EXISTING) != FR_OK)
                {
                        ...
                        {
                                ClearScreen();
                                ShowMessageScreenAndExit("Failed to create Memor
y Card File!", 1);
                        }
                }
                else
        ...
        if (!(ncfg->Config & (NIN_CFG_SKIP_IPL)))
        {
                if(IsTRIGame == 0)
                {
                        ...
                        {
                                case BI2_REGION_USA:
                                        snprintf(iplchar, sizeof(iplchar), "%s:/
iplusa.bin", GetRootDevice());
                                        break;
                                ...
                                default:
                                        snprintf(iplchar, sizeof(iplchar), "%s:/
ipljap.bin", GetRootDevice());
                                        break;

                                case BI2_REGION_PAL:
                                        // FIXME: PAL IPL is broken on Wii U.
                                        if (!IsWiiU())
                                                snprintf(iplchar, sizeof(iplchar
), "%s:/iplpal.bin", GetRootDevice());
                                        break;
                        }

                        FIL f;
                        if (iplchar[0] != 0 &&
                            f_open_char(&f, iplchar, FA_READ|FA_OPEN_EXISTING) =
= FR_OK)
                        {
                ...
                else
                {
                        ...
                        char iplchar[32];
                        snprintf(iplchar, sizeof(iplchar), "%s:/segaboot.bin", G
etRootDevice());
                        FIL f;
                        if (f_open_char(&f, iplchar, FA_READ|FA_OPEN_EXISTING) =
= FR_OK)
                        {
                                if (f.obj.objsize == TRI_IPL_SIZE)
                                {
                                        f_lseek(&f, 0x20);
                                        void *iplbuf_tri = (void*)0x92A80000; //
 Different buffer for Triforce IPL
                                        UINT read;
                                        f_read(&f, iplbuf_tri, TRI_IPL_SIZE - 0x
20, &read);
                                        useipltri = (read == (TRI_IPL_SIZE - 0x2
0));
                                }
                                f_close(&f);
                        }
        ...
        if( (ncfg->Config & NIN_CFG_FORCE_PROG) &&
                        ncfg->GameID == 0x47584745) // GXGE (Mega Man X Collecti
on USA)
...
+ 20*6, "Loading patched kernel... %d", STATUS_LOADING);
                        if(STATUS_LOADING == 0)
                        {
                                PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MEN
U_POS_Y + 20*7, "ES_Init...");
                                // Cleans the -1 when it's past it to avoid conf
usion if another error happens. e.g. before it showed "81" instead of "8" if the
 controller was unplugged.
                                PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X + 16
3, MENU_POS_Y + 20*6, " ");
                        }
                        if((STATUS_LOADING > 0 || abs(STATUS_LOADING) > 1) && ST
ATUS_LOADING < 20)
                                PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MEN
U_POS_Y + 20*7, "ES_Init... Done!");
                        if(STATUS_LOADING == 2)
                                PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MEN
U_POS_Y + 20*8, "Initing storage devices...");
                        if(abs(STATUS_LOADING) > 2 && abs(STATUS_LOADING) < 20)
                                PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MEN
U_POS_Y + 20*8, "Initing storage devices... Done!");
                        if(STATUS_LOADING == -2)
                                PrintFormat(DEFAULT_SIZE, MAROON, MENU_POS_X, ME
NU_POS_Y + 20*8, "Initing storage devices... Error! %d  Shutting down", STATUS_E
RROR);
                        if(STATUS_LOADING == 3)
                                PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MEN
U_POS_Y + 20*9, "Mounting USB/SD device...");
                        if(abs(STATUS_LOADING) > 3 && abs(STATUS_LOADING) < 20)
                                PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MEN
U_POS_Y + 20*9, "Mounting USB/SD device... Done!");
                        if(STATUS_LOADING == -3)
                                PrintFormat(DEFAULT_SIZE, MAROON, MENU_POS_X, ME
NU_POS_Y + 20*9, "Mounting USB/SD device... Error! %d  Shutting down", STATUS_ER
ROR);
                        if(STATUS_LOADING == 5) {
        /*                      if (timeout == 0)
                                        timeout = ticks_to_secs(gettime()) + 20;
 // Set timer for 20 seconds
                                        ...
                                        //DCFlushRange( (void*)0x92f00000, 0x100
000 );
                                        //ExitToLoader(1);
                                }*/
                                PrintFormat(DEFAULT_SIZE, (STATUS_ERROR == -7) ?
 MAROON:BLACK, MENU_POS_X, MENU_POS_Y + 20*10, (STATUS_ERROR == -7) ? "Checking
FS... Timeout!" : "Checking FS...");
                        }
                        if(abs(STATUS_LOADING) > 5 && abs(STATUS_LOADING) < 20)
                        {
                                PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MEN
U_POS_Y + 20*10, "Checking FS... Done!");
                                PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MEN
U_POS_Y + 20*11, "Drive size: %.02f%s Sector size: %d", STATUS_DRIVE, STATUS_GB_
MB ? "GB" : "MB", STATUS_SECTOR);
                        }
                        if(STATUS_LOADING == -5)
                                PrintFormat(DEFAULT_SIZE, MAROON, MENU_POS_X, ME
NU_POS_Y + 20*10, "Checking FS... Error! %d Shutting down", STATUS_ERROR);
                        if(STATUS_LOADING == 6)
                                PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MEN
U_POS_Y + 20*12, "ES_LoadModules...");
                        if(abs(STATUS_LOADING) > 6 && abs(STATUS_LOADING) < 20)
                                PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MEN
U_POS_Y + 20*12, "ES_LoadModules... Done!");
                        if(STATUS_LOADING == -6)
                                PrintFormat(DEFAULT_SIZE, MAROON, MENU_POS_X, ME
NU_POS_Y + 20*12, "ES_LoadModules... Error! %d Shutting down", STATUS_ERROR);
                        if(STATUS_LOADING == 7)
                                PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MEN
U_POS_Y + 20*13, "Loading config...");
                        if(abs(STATUS_LOADING) > 7 && abs(STATUS_LOADING) < 20)
                                PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MEN
U_POS_Y + 20*13, "Loading config... Done!");
                        /*if(STATUS_LOADING == 8)
                        {
                                PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MEN
U_POS_Y + 20*14, "Init HID devices... ");
                                if ( STATUS_ERROR == 1)
                                {
                                        PrintFormat(DEFAULT_SIZE, BLACK, MENU_PO
S_X, MENU_POS_Y + 20*15, "          Make sure the Controller is plugged in");
                                }
                                else
                                        PrintFormat(DEFAULT_SIZE, BLACK, MENU_PO
S_X, MENU_POS_Y + 20*15, "%50s", " ");
                        }
                        if(abs(STATUS_LOADING) > 8 && abs(STATUS_LOADING) < 20)
                        {
                                if (ncfg->Config & NIN_CFG_NATIVE_SI)
                                        PrintFormat(DEFAULT_SIZE, BLACK, MENU_PO
S_X, MENU_POS_Y + 20*14, "Init HID devices... Using ONLY NATIVE Gamecube Ports")
;
                                else if ((ncfg->MaxPads == 1) && (ncfg->Config &
 NIN_CFG_HID))
                                        PrintFormat(DEFAULT_SIZE, BLACK, MENU_PO
S_X, MENU_POS_Y + 20*14, "Init HID devices... Using Gamecube and HID Ports");
                                else if ((ncfg->MaxPads > 0) && (ncfg->Config &
NIN_CFG_HID))
                                        PrintFormat(DEFAULT_SIZE, BLACK, MENU_PO
S_X, MENU_POS_Y + 20*14, "Init HID devices... Using Gamecube, HID, and BT Ports"
);
                                else if (ncfg->MaxPads > 0)
                                        PrintFormat(DEFAULT_SIZE, BLACK, MENU_PO
S_X, MENU_POS_Y + 20*14, "Init HID devices... Using Gamecube and BT Ports");
                                else if (ncfg->Config & NIN_CFG_HID)
                                        PrintFormat(DEFAULT_SIZE, BLACK, MENU_PO
S_X, MENU_POS_Y + 20*14, "Init HID devices... Using HID and Bluetooth Ports");
                                else
                                        PrintFormat(DEFAULT_SIZE, BLACK, MENU_PO
S_X, MENU_POS_Y + 20*14, "Init HID devices... Using Bluetooth Ports... Done!");
                        }
                        if(STATUS_LOADING == -8)
                        {
                                PrintFormat(DEFAULT_SIZE, MAROON, MENU_POS_X, ME
NU_POS_Y + 20*14, "Init HID devices... Failed! Shutting down");
                                switch (STATUS_ERROR)
                                {
                                        case -1:
                                                PrintFormat(DEFAULT_SIZE, MAROON
, MENU_POS_X, MENU_POS_Y + 20*15, "No Controller plugged in! %25s", " ");
                                                break;
                                        case -2:
                                                PrintFormat(DEFAULT_SIZE, MAROON
, MENU_POS_X, MENU_POS_Y + 20*15, "Missing %s:/controller.ini %20s", GetRootDevi
ce(), " ");
                                                break;
                                        case -3:
                                                PrintFormat(DEFAULT_SIZE, MAROON
, MENU_POS_X, MENU_POS_Y + 20*15, "Controller does not match %s:/controller.ini
%6s", GetRootDevice(), " ");
                                                break;
                                        case -4:
                                                PrintFormat(DEFAULT_SIZE, MAROON
, MENU_POS_X, MENU_POS_Y + 20*15, "Invalid Polltype in %s:/controller.ini %12s",
 GetRootDevice(), " ");
                                                break;
                                        case -5:
                                                PrintFormat(DEFAULT_SIZE, MAROON
, MENU_POS_X, MENU_POS_Y + 20*15, "Invalid DPAD value in %s:/controller.ini %9s"
, GetRootDevice(), " ");
                                                break;
                                        case -6:
                                                PrintFormat(DEFAULT_SIZE, MAROON
, MENU_POS_X, MENU_POS_Y + 20*15, "PS3 controller init error %25s", " ");
                                                break;
                                        case -7:
                                                PrintFormat(DEFAULT_SIZE, MAROON
, MENU_POS_X, MENU_POS_Y + 20*15, "Gamecube adapter for Wii u init error %13s",
" ");
                                                break;
                                        default:
                                                PrintFormat(DEFAULT_SIZE, MAROON
, MENU_POS_X, MENU_POS_Y + 20*15, "Unknown error %d %35s", STATUS_ERROR, " ");
                                                break;
                                }
                        }*/
                        if(STATUS_LOADING == 9)
                                PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MEN
U_POS_Y + 20*14, "Init DI... %40s", " ");
                        if(abs(STATUS_LOADING) > 9 && abs(STATUS_LOADING) < 20)
                                PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MEN
U_POS_Y + 20*14, "Init DI... Done! %35s", " ");
                        if(STATUS_LOADING == 10)
                                PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MEN
U_POS_Y + 20*15, "Init CARD...");
                        if(abs(STATUS_LOADING) > 10 && abs(STATUS_LOADING) < 20)
                                PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MEN
U_POS_Y + 20*15, "Init CARD... Done!");
                        GRRLIB_Screen2Texture(0, 0, screen_buffer, GX_FALSE); //
 Copy all status messages
                        GRRLIB_Render();
                        ClearScreen();
                }
                while((STATUS_LOADING < -1) && (STATUS_LOADING > -20)) //display
ing a fatal error
        ...
        if(argsboot == false)
        {
                ...
                PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*17,
 "Nintendont kernel looping, loading game...");
                GRRLIB_Render();
                DrawBuffer(); // Draw all status messages
        }
//      memcpy( (void*)0x80000000, (void*)0x90140000, 0x1200000 );
    ...
    bool forceProgressive = (ncfg->Config & NIN_CFG_FORCE_PROG) && !useipl && !u
seipltri;
    ...
    else if (generalForceVID)
    {
        if (specificForceFlags & NIN_VID_FORCE_NTSC) {
            ...
            *(vu32*)0x800000CC = 1; vmode = &TVPal576ProgScale; // Use compiler
suggested TVPal576ProgScale
        }
    }
    // --- VIDEO MODE LOGIC END ---
        ...
        if((ncfg->Config & NIN_CFG_MEMCARDEMU) == 0)
        {
                ...
                sram->flags             &= ~0x80;       // Clear Progmode initia
lly
        ...
        bool spPopWW = (ncfg->GameID == 0x47324F45 && ncfg->Language == NIN_LAN_
SPANISH);
        if ((ncfg->GameID >> 8) != 0x474233 && !spPopWW && (ncfg->VideoMode & NI
N_VID_PROG))
        {
            sram->flags |= 0x80;
        }
        ...
        *(vu32*)0x93003008 = 0x80000004;
        ...
        else
        {
                ...
                memcpy((void*)0x81300000, multidol_ldr_bin, multidol_ldr_bin_siz
e);
