...
static s32 __USB_BlkMsgTimeout(usbstorage_handle *dev, u8 bEndpoint, u16 wLength, void *rpData, u32 timeout)
{
	...
	retval = USB_OGC_WriteBlkMsgAsync(dev->usb_fd, bEndpoint, wLength, rpData, __usb_blkmsg_cb, (void *)dev);
	...
	if (retval<0)
		USB_OGC_ClearHalt(dev->usb_fd, bEndpoint);
...
static s32 __USB_CtrlMsgTimeout(usbstorage_handle *dev, u8 bmRequestType, u8 bmRequest, u16 wValue, u16 wIndex, u16 wLength, void *rpData)
{
	...
	dev->retval = USBSTORAGE_PROCESSING;
	retval = USB_OGC_WriteCtrlMsgAsync(dev->usb_fd, bmRequestType, bmRequest, wValue, wIndex, wLength, rpData, __usb_blkmsg_cb, (void *)dev);
	if(retval < 0) return retval;
...
s32 USBStorageOGC_Suspend(usbstorage_handle *dev)
{
	...
	USB_OGC_SuspendDevice(dev->usb_fd);
	dev->suspended = 1;
...
{
	if(USB_OGC_Initialize() < 0 || USBStorageOGC_Initialize() < 0)
		return false;
...
static bool __usbstorage_ReadSectors(u32 sector, u32 numSectors, void *buffer)
{
	...
	retval = USBStorageOGC_Read(&__usbfd, __lun, sector, numSectors, buffer);
...
static bool __usbstorage_WriteSectors(u32 sector, u32 numSectors, const void *buffer)
{
	...
	retval = USBStorageOGC_Write(&__usbfd, __lun, sector, numSectors, buffer);
...
DISC_INTERFACE __io_custom_usbstorage = {
	DEVICE_TYPE_WII_USB,
